...
# Semaphore to control concurrency (OpenAI rate limits vary by tier)
...
async def get_gpt_response(model, prompt, prompt_id, language, variant_name, settings, pass_index):
    ...
    async with semaphore:
        ...
        for attempt in range(retries):
            try:
                ...
                if len(result_buffer) >= 50: await save_buffer()
                return result_data
                ...
                if e.status_code == 429:
                    ...
                    # If it's a content filter (400/403), we might want to record it rather than retry forever
                    if e.status_code == 400 and "content_filter" in str(e):
                         return {
                            ...
                            "response": "[BLOCKED_BY_OPENAI_FILTER]", "finish_reason": "content_filter",
