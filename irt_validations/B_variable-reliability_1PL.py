# -*- coding: utf-8 -*-
"""
REMOVE AFTER 2PL CODE FINALIZED
...
KNOWN ISSUE: This script fits a 1PL (no α_i discrimination parameter),
while the paper states 2PL was adopted after Experiment A. Either port
model_2pl here or explicitly note 1PL in the paper's reliability section.
"""
...
def fit_irt(df_subset, anchor_ids, label="full"):
    """
    Single unified IRT fitter with explicit shape enforcement.
    Used by ALL sub-experiments (B1–B7).
    """
...
def b5_empirical_icc_validation(df, consistency_df, anchor_ids):
    ...
    for _, row in tqdm(consistency_df.iterrows(), total=len(consistency_df), desc="  Validating", leave=False):
        ...
        logit = theta_avg - (beta_i + gamma_l + tau_il)
    ...
    for idx, pid in enumerate(example_prompts):
        ...
        if pid in results['prompt_map']:
            ...
            ax.plot(theta_range, 1/(1+np.exp(-(theta_range - diff_en))), 'k-', linewidth=2, label='IRT (en)')
            for show_lang in ['bn', 'jv', 'sw']:
                if show_lang in results['lang_map']:
                    ...
                    ax.plot(theta_range, 1/(1+np.exp(-(theta_range - diff_l))), '--', linewidth=1.5, label=f'IRT ({show_lang})')
                    break
