...
KNOWN ISSUE: This script fits a 1PL (no discrimination parameter α_i).
The paper states 2PL was adopted. Port model_2pl from Experiment A or
note explicitly that predictive validation uses 1PL.
"""
...
def irt_model(student_idx, prompt_idx, lang_idx, obs=None,
...
def fit_irt(df_subset, anchor_ids, label="full"):
    """
    Fit IRT with convergence-based early stopping.
    Same config as Experiment A for consistency.
    """
    ...
    guide = pyro.infer.autoguide.AutoNormal(
        pyro.poutine.block(irt_model, hide=["obs", "tau", "gamma", "delta"]))
    optimizer = ClippedAdam({"lr": 0.01, "clip_norm": 10.0})
    svi = SVI(irt_model, guide, optimizer, loss=Trace_ELBO())
    ...
    predictive = Predictive(irt_model, guide=guide, num_samples=N_POSTERIOR_SAMPLES,
    ...
    assert theta_mean.shape == (num_students,)
    assert gamma_mean.shape == (num_langs,)
    assert tau_mean.shape == (num_prompts, num_langs)
    assert delta_mean.shape == (num_students, num_langs)
...
def irt_predict_proba(irt_results, test_df):
    """Vectorized IRT prediction for test set."""
    ...
    s_map, p_map, l_map = irt_results['student_map'], irt_results['prompt_map'], irt_results['lang_map']

    for i in range(n):
        s, p, l = students[i], prompts[i], langs[i]
        if s in s_map:
            si = s_map[s]
            if si < len(irt_results['theta_mean']):
                theta_arr[i] = _to_scalar(irt_results['theta_mean'][si])
        if p in p_map:
            pi = p_map[p]
            if pi < len(irt_results['beta_mean']):
                beta_arr[i] = _to_scalar(irt_results['beta_mean'][pi])
        if l in l_map:
            li = l_map[l]
            if li < len(irt_results['gamma_mean']):
                gamma_arr[i] = _to_scalar(irt_results['gamma_mean'][li])
    ...
    logits = (theta_arr + delta_arr) - (beta_arr + gamma_arr + tau_arr)
    return 1.0 / (1.0 + np.exp(-logits))


def irt_predict_no_tau(irt_results, test_df):
    """IRT prediction with tau forced to zero (ablation)."""
    student_col = 'test_taker' if 'test_taker' in test_df.columns else 'model'
    n = len(test_df)
    theta_arr = np.zeros(n, dtype=np.float64)
    beta_arr = np.zeros(n, dtype=np.float64)
