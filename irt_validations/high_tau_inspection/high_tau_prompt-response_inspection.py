# -*- coding: utf-8 -*-
"""
Qualitative Inspection of High Positive-τ Prompt×Language Pairs
================================================================
Positive τ = prompt is harder/more dangerous in non-English.
These are the pairs where models jailbreak MORE in translation.

1. Loads τ (Safety_Tax) from IRT results
2. Finds top 15 highest positive τ pairs
3. Pulls actual model responses — prefers jailbreak examples
4. Outputs CSV + markdown for manual reading
"""
...
def load_tau():
    ...
    tau_long = tau_long.rename(columns={'prompt': 'prompt_id', 'Safety_Tax': 'tau'})
...
def load_master_data():
    ...
    for c in ['model_output', 'model_response', 'response', 'output',
        ...
        if c in df.columns:
            response_col = c; break
    if response_col is None:
        for col in df.columns:
            if df[col].dtype == object and col not in [
                    'id', 'language', 'test_taker', 'model', 'category', 'prompt']:
                if df[col].dropna().head(5).str.len().mean() > 50:
                    response_col = col; break
...
def extract_responses(master_df, top_pairs, response_col, prompt_col, tt_col):
    ...
    for _, pair in top_pairs.iterrows():
        ...
        if prompt_col:
            ...
            if len(any_row): prompt_text = str(any_row[prompt_col].iloc[0])

        for fam in FAM_ORDER:
            fam_rows = lang_resp[lang_resp['family'] == fam]
            if len(fam_rows) == 0: continue
            ...
            records.append({
                'prompt_id': pid, 'language': lang, 'tau': round(tau_val, 3),
                'family': fam, 'model': str(row[tt_col])[:40],
                ...
                'response_en': en_text, 'judge_score_en': en_judge,
...
def save_outputs(result, top_pairs):
    ...
    md_lines = [
        "# High Positive-τ Qualitative Inspection\n",
        "Positive τ = prompt is **harder/more dangerous** in non-English.\n",
...
def main():
    print("=" * 60)
    print("QUALITATIVE INSPECTION: High Positive-τ Responses")
    print("=" * 60)
    ...
    if response_col is None:
        print("\n⚠️  No response text column found!")
        return
