...
def compute_lang_ranks(lang_df, irt_model):
    ...
    for lang, grp in df.groupby('language'):
        grp = grp.copy()
        grp['JSR_Rank']        = grp['JSR_lang'].rank(ascending=False, method='min').astype(int)
        grp['ThetaDelta_Rank'] = grp['theta_minus_delta'].rank(ascending=True, method='min').astype(int)
...
def plot_top_movers(overall, irt_model, top_k=20):
    ...
    ax.set_title(f'Top {len(top)} Rank Divergences ({irt_model}) — XSafety')
...
def plot_divergence_distribution(overall, irt_model):
    ...
    qwk = quadratic_weighted_kappa(df['JSR_Rank'].values, df['Theta_Rank'].values)
    mad = mean_absolute_rank_shift(df['JSR_Rank'].values, df['Theta_Rank'].values)
    ...
    metrics_text = (
        ...
        f"|Delta|>=5: {(np.abs(deltas) >= 5).sum()}/{n}\n"
        f"|Delta|>=10: {(np.abs(deltas) >= 10).sum()}/{n}"
    )
...
# ══════════════════════════════════════════════════════════════════════════
# PLOT 3 — Family-Level Divergence
# ══════════════════════════════════════════════════════════════════════════
...
def plot_family_divergence(overall, irt_model):
    ...
    for i, fam in enumerate(present_fams):
        ...
        ax.scatter(np.full_like(fam_data, i + 1, dtype=float) + jitter,
                   ...
                   s=10, alpha=0.7, edgecolors='black', linewidths=0.2, zorder=3)
    ...
    if fam_metrics:
        fm_df  = pd.DataFrame(fam_metrics).sort_values('RMS_shift', ascending=True)
        colors = [FAM_COLORS.get(f, '#7f8c8d') for f in fm_df['family']]
        bars   = ax.barh(fm_df['family'], fm_df['RMS_shift'], color=colors,
                         edgecolor='black', linewidth=0.3)
        for bar, val, md in zip(bars, fm_df['RMS_shift'], fm_df['mean_delta']):
            ax.text(bar.get_width() + 0.1,
                    bar.get_y() + bar.get_height() / 2,
                    f'{val:.1f} ({md:+.1f})', va='center', fontsize=4)
...
def plot_language_divergence(lang_df, irt_model):
    ...
    for lang in languages:
        ...
        if len(grp) < 3:
            continue
        rm  = rmsrd(grp['JSR_Rank'].values, grp['ThetaDelta_Rank'].values, len(grp))
        qwk = quadratic_weighted_kappa(grp['JSR_Rank'].values, grp['ThetaDelta_Rank'].values)
        rho, _ = spearmanr(grp['JSR_Rank'].values, grp['ThetaDelta_Rank'].values)
        mad = mean_absolute_rank_shift(grp['JSR_Rank'].values, grp['ThetaDelta_Rank'].values)
        lang_metrics.append({
    ...
    ax.barh(lm_df['language'], lm_df['QWK'], color=colors_qwk,
            edgecolor='black', linewidth=0.3)
    ax.axvline(0.8, color=_c1, linestyle='--', alpha=0.5, lw=0.5, label='Excellent')
    ax.axvline(0.6, color=_c3, linestyle='--', alpha=0.5, lw=0.5, label='Moderate')
    for i, (_, row) in enumerate(lm_df.iterrows()):
        ax.text(max(0, row['QWK']) + 0.005, i, f"{row['QWK']:.3f}",
                va='center', fontsize=4)
    ax.set_xlabel('QW kappa')
    ...
    fig.tight_layout()
    path = os.path.join(RESULTS_DIR, f"language_divergence_{irt_model}.png")
    _save(fig, path)
    print(f"  Saved: {os.path.basename(path)}")

    lm_df.to_csv(os.path.join(RESULTS_DIR,
                               f"language_divergence_metrics_{irt_model}.csv"),
                 index=False)
    return lm_df


# ══════════════════════════════════════════════════════════════════════════
# PLOT 5 — Family × Language Heatmap
# ══════════════════════════════════════════════════════════════════════════
...
def plot_family_lang_heatmap(lang_df, irt_model):
    ...
    sns.heatmap(pivot, cmap=_cmap, center=0, vmin=-vmax, vmax=vmax,
                ...
                cbar_kws={'label': 'Mean Rank Delta', 'shrink': 0.8}, ax=ax)
    ax.set_title(f'Family × Language Mean Rank Shift ({irt_model}) — XSafety')
    ax.set_ylabel('')
    ax.set_xlabel('Language')

    path = os.path.join(RESULTS_DIR, f"family_lang_heatmap_{irt_model}.png")
    _save(fig, path)
    print(f"  Saved: {os.path.basename(path)}")
...
def save_rank_tables(overall, lang_df, irt_model):
    ...
    path = os.path.join(RESULTS_DIR, f"rank_table_overall_{irt_model}.csv")
    ...
    if len(lang_df) > 0 and irt_model in lang_df['irt_model'].values:
        ...
        if len(ldf) > 0:
            ...
            path_l = os.path.join(RESULTS_DIR,
                                   f"rank_table_per_language_{irt_model}.csv")
...
def main():
    if _HAS_FIG_STYLE:
        apply_style()
    print("=" * 60)
    print("RANK DIVERGENCE ANALYSIS — XSafety")
    print("=" * 60)
    ...
    for _, row in summary_df.iterrows():
        print(f"  [{row['irt_model']}]  RMSRD={row['RMSRD']:.3f}  "
              ...
              f"rho={row['Spearman_rho']:.3f}  N={row['N']}")
