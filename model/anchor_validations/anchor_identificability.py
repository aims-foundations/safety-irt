...
def make_model(tau_prior='studentt', tau_df=1.0, anchor_scale=0.01):
    """
    Returns a 2PL model function with the specified τ prior.
    Anchors get τ ~ N(0, anchor_scale) — soft constraint, not hard zero.
    English column remains hard zero via tau_mask (identification constraint).
    """
    def model_fn(student_idx, prompt_idx, lang_idx, obs=None,
        ...
        # ── τ prior: soft anchoring ──
        # Anchors: N(0, anchor_scale=0.01) — tight but not pinned
        # Non-anchors: specified prior (StudentT or Normal)
        # English column: hard zero via tau_mask (identification constraint)
        if tau_prior == 'studentt':
            tau_scale_global = pyro.sample("tau_scale",
                dist.HalfCauchy(
                    torch.ones(1, device=device)).to_event(1))
            tau_scale_per = torch.where(
                anchor_mask_tensor > 0.5,
                torch.full((num_prompts, num_langs), anchor_scale, device=device),
                tau_scale_global.expand(num_prompts, num_langs))
            ...
            tau_scale_per = torch.where(
                anchor_mask_tensor > 0.5,
                torch.full((num_prompts, num_langs), anchor_scale, device=device),
                torch.ones(num_prompts, num_langs, device=device))
            tau_raw = pyro.sample("tau_raw",
                dist.Normal(
                    torch.zeros(num_prompts, num_langs, device=device),
                    tau_scale_per).to_event(2))
...
def fit_variant(df, anchor_ids, prior_name, prior_cfg):
    ...
    if os.path.exists(ANCHOR_FILE):
        ...
        id_col = 'prompt_id' if 'prompt_id' in adf.columns else 'id'
        if 'is_anchor' in adf.columns:
            adf = adf[adf['is_anchor'] == True]
        adf['id'] = adf[id_col].apply(clean_id)
    ...
    model_fn = make_model(prior_cfg['dist'], prior_cfg.get('df', 1.0), anchor_scale=0.01)
...
def load_data():
    ...
    if os.path.exists(ANCHOR_FILE):
        ...
        if 'is_anchor' in adf.columns:
            adf = adf[adf['is_anchor'] == True]
        anchor_ids = set(adf[id_col].apply(clean_id).unique())
