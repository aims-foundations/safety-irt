...
OUTPUT_CSV_DATA = os.path.join(RESULTS_DIR, "embedding_analysis_translation_v_safety.csv")
OUTPUT_CSV_LANG = os.path.join(RESULTS_DIR, "embedding_analysis_translation_v_safety_Lang.csv")
OUTPUT_CSV_CAT = os.path.join(RESULTS_DIR, "embedding_analysis_translation_v_safety_Cat.csv")
...
def compute_embeddings(df):
    """Computes LaBSE embeddings and Cosine Similarity."""
...
def compute_statistics(df):
    ...
    for lang, sub in df.groupby("language"):
        if len(sub) > 15:
            r, l, h = bootstrap_spearman_ci(sub["similarity"], sub["safety_rate"], n_boot=2000)
            lang_rows.append({
                "Language": lang,
                "N": len(sub),
                "Spearman_Rho": r,
...
# --- MAIN PIPELINE ---
def run_pipeline():
    # 1. Load & aggregate (now includes category)
