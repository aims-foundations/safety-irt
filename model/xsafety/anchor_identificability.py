# -*- coding: utf-8 -*-
"""
Horseshoe Sensitivity & γ-τ Multicollinearity Analysis — XSafety
=================================================================
Adapted from model/anchor_validations/anchor_identificability.py for XSafety:
  - Single pass only (no pass column)
  - Input: XSafety_Dataset.csv
  - Anchors from local results_dif_stratified/soft_anchor_priors.csv
...
Reads:
   - XSafety_Dataset.csv (raw data)
   - results_dif_stratified/soft_anchor_priors.csv (anchors)

Produces:
   - horseshoe_gamma_sensitivity.png — γ comparison across priors
   - horseshoe_tau_sensitivity.png  — τ distribution under each prior
...
# Anchors generated by xsafety/anchors.py
...
def plot_gamma_comparison(results):
    ...
    plt.suptitle('Horseshoe Sensitivity: Effect on γ Estimates (XSafety)',
...
def plot_tau_comparison(results):
    ...
    # Right: per-language mean |τ|
    ax = axes[2]
    ref    = results['horseshoe']
    l_map  = ref['lang_map']
    non_en = [l for l in sorted(l_map.keys(), key=lambda x: l_map[x]) if l != 'en']
    ...
    plt.suptitle('Horseshoe Sensitivity: Effect on τ Estimates (XSafety)',
                 fontsize=14, fontweight='bold')
...
def plot_gamma_tau_collinearity(results):
    ...
    plt.suptitle('Multicollinearity Check: γ_L vs mean(τ_iL) — XSafety\n'
...
def main():
    if _HAS_FIG_STYLE:
        apply_style()
    print("=" * 60)
    print("HORSESHOE SENSITIVITY & γ-τ MULTICOLLINEARITY — XSafety")
    print("=" * 60)
