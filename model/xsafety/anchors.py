# -*- coding: utf-8 -*-
"""
Stratified-Variance + Agreement-Rank Anchor Selection — XSafety
================================================================
Identical pipeline to model/anchors.py, adapted for XSafety:
  - 9 focal languages: zh, ar, bn, de, fr, hi, ja, ru, sp
  - Single pass (no pass column) — person_key = test_taker directly
  ...
  2. Agreement-rank selection
     Lord's χ²(2) averaged across all 9 focal languages.
     Select top N_ANCHORS by lowest mean χ².
...
REFERENCE_LANG = "en"
# XSafety focal languages (no it, vi, ko, th, sw, jv)
ALL_LANGS      = ["zh", "ar", "bn", "de", "fr", "hi", "ja", "ru", "sp"]
...
def load_response_matrices():
    # XSafety: single pass — no 'pass' column, person_key = test_taker directly
    df = pd.read_csv(INPUT_FILE, engine="python", on_bad_lines="skip",
                     usecols=["id", "language", "test_taker", "judge_score"])
    ...
    df["person_key"] = df["test_taker"].astype(str)
...
def main():
    print("=" * 70)
    print("XSAFETY — STRATIFIED-VARIANCE + AGREEMENT-RANK ANCHOR SELECTION")
