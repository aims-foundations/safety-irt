# -*- coding: utf-8 -*-
"""
Multi-Metric Translation Quality Analysis vs Safety Cost (Tau) — XSafety.

Adapted from model/embedding_analysis_translation_v_CSG.py for XSafety:
  - Uses XSafety_Dataset.csv (single pass, 10 languages)
  - prompt_en column used as English reference (no separate multijail.csv needed)
  - category column is a single string (no parse_tags_cell needed)
  - NON_EN_LANGS set to XSafety focal languages
...
# XSafety focal languages (all non-English)
NON_EN_LANGS = ["zh", "ar", "bn", "de", "fr", "hi", "ja", "ru", "sp"]
...
def build_pairs():
    ...
    # XSafety has prompt_en (English original) and prompt (translated) on every row
    has_prompt_en = "prompt_en" in raw_df.columns
    if has_prompt_en:
        # Use prompt_en directly as reference, prompt as translated text
        text_lookup = raw_df[["id", "language", "prompt", "prompt_en"]].drop_duplicates(
            subset=["id", "language"]
        )
        eng_lookup = text_lookup.set_index("id")["prompt_en"].drop_duplicates().to_dict()
        print(f"  Using prompt_en column for English reference. {len(eng_lookup)} unique prompts.")
    else:
        # Fallback: extract English rows and match by id
    ...
    for _, row in target_rows.iterrows():
        ...
        meta_rows.append({
            "language": lang, "id": p_id, "category": category,
...
def compute_correlations_and_plot():
    ...
    # --- Per-Category correlations (XSafety: category is a single string) ---
    cat_rows = []
    for metric in available_metrics:
        for cat in sorted(df["category"].dropna().unique()):
            sub = df[(df["category"] == cat) & df[metric].notna() & df["tau"].notna()].copy()
            if len(sub) > 10:
                r, p = spearmanr(sub[metric], sub["tau"])
                cat_rows.append({
                    "Metric":      METRIC_LABELS.get(metric, metric),
    ...
    print(summary_df[[
        "Metric", "Mean_PerLanguage_Rho", "Median_PerLanguage_Rho",
        "WeightedMean_PerLanguage_Rho", "Positive_Languages",
        "Negative_Languages", "Sig_Positive_Languages",
        "Sig_Negative_Languages", "Pooled_Rho_AllRows",
    ...
    sns.heatmap(
        pivot,
        annot=True, fmt=".3f", cmap=CMAP_DIV, center=0, ax=ax,
        linewidths=0.5,
        cbar_kws={"label": LABELS["rho"], "shrink": 0.8}
    )
    ax.set_title("Per-language ρ (XSafety)")
    ax.set_ylabel("")
