# -*- coding: utf-8 -*-
"""
Multi-Metric Translation Quality Analysis vs Safety Rate — XSafety.

Adapted from model/embedding_analysis_translation_v_safety.py for XSafety:
  - Uses XSafety_Dataset.csv (single pass, 10 languages)
  - category column is a single string (no parse_tags_cell needed)
  - prompt_en column used as English reference where available
...
def load_and_aggregate():
    ...
    # Attach prompt_en if available (XSafety stores English original on every row)
    if "prompt_en" in raw_df.columns:
        en_ref = raw_df[["id", "prompt_en"]].drop_duplicates(subset=["id"])
        agg_df = agg_df.merge(en_ref, on="id", how="left")
...
def build_pairs(df):
    ...
    has_prompt_en = "prompt_en" in df.columns

    if has_prompt_en:
        eng_lookup = df[["id", "prompt_en"]].drop_duplicates(subset=["id"]).set_index("id")["prompt_en"].to_dict()
        print(f"  Using prompt_en column. {len(eng_lookup)} English reference prompts.")
    else:
    ...
    text_lookup = df[["id", "language", "prompt"]].drop_duplicates(subset=["id", "language"]).copy()
...
def compute_correlations_and_plot():
    ...
    # --- Per-Category with bootstrap CIs (XSafety: category is a single string) ---
    cat_rows = []
    for metric in available_metrics:
    ...
    for metric in available_metrics:
        ...
        print(f"  {label:12s}: rho={r:.3f} [{lo:.3f}, {hi:.3f}]")
    ...
    axes[0].barh(global_df["Metric"], global_df["Spearman_Rho"], color=colors)

    for i, (_, row) in enumerate(global_df.iterrows()):
        axes[0].plot([row["CI_Lower"], row["CI_Upper"]], [i, i], color="black", linewidth=1.5)

    axes[0].axvline(0, color="black", linewidth=1)
    axes[0].set_title("Global: Metric vs Safety Rate (XSafety)", fontsize=14, fontweight="bold")
    axes[0].set_xlabel("Spearman's rho (with 95% CI)")
    ...
    sns.heatmap(pivot, annot=True, fmt=".3f", cmap=CMAP_DIV, center=0,
                ax=axes[1], linewidths=0.5, cbar_kws={"label": "Spearman rho"})
